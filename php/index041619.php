...
$ventasI = mysql_query("SELECT (SELECT SUM(total) FROM ventas_articulos WHERE idventa=ventas.idventas - ventas.descuento) as total FROM ventas WHERE ventas.fecha = CURDATE()");
$tventas = 0;
while ($a = mysql_fetch_object($ventasI)) {
	$tventas += $a->total;
}

$pedidos = mysql_num_rows(mysql_query("SELECT idventa FROM destinatarios WHERE fechaentrega=CURDATE()"));
...
<?php
					$query = mysql_query("SELECT * FROM destinatarios JOIN ventas ON ventas.idventas=destinatarios.idventa WHERE fechaentrega=CURDATE() ORDER BY idventa ASC");
